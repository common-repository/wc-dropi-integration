...
class JPIODFW_WC_States_Places_Colombia_Dropi
{
    ...
    public function __construct($file)
    {
        ...
        add_filter('woocommerce_default_address_fields', array( $this, 'states_places_colombia_smp_woocommerce_default_address_fields') , 1000, 1);
    }
    ...
    public function  wc_states($states)
    {
        ...
        if (!empty($allowed)) {
            foreach ($allowed as $code => $country) {
               ...
               //echo self::get_plugin_path() . '/states/' . $code . '.php'
    ...
    public function load_scripts()
    {
        if (is_cart() || is_checkout() || is_wc_endpoint_url('edit-address')) {
            ...
            $city_select_path = self::get_plugin_url() . '../js/place-select.js?'.date('ymdHis');
    ...
    private static function get_plugin_path()
    {
        ...
        $path = self::$plugin_path = dirname(__DIR__);
